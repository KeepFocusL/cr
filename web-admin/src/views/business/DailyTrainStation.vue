...
const rules = {
  ...
  km: [
    {required: true, message: '请输入里程（公里）', trigger: 'blur'},
  ],
}
...
const handleDelete = async (ids) => {
  try {
    const res = await deleteDailyTrainStation(ids)
    if (res.code === 200) {
      ElMessage.success('删除成功')
      // 如果当前页只剩要被删除的条数且不是第一页，删除后自动跳转到上一页
      if (dailyTrainStationList.value.length === ids.length && pagination.page > 1) {
        pagination.page--
      }
      await getDailyTrainStationList()
    } else {
...
const getDailyTrainStationList = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.msg || '获取每日火车车站列表失败')
  } finally {
...
<template>
  <div class="dailyTrainStation-container">
    ...
    >
      ...
      <el-table-column prop="km" label="里程（公里）"/>
    ...
    >
      ...
      >
        ...
        <el-form-item label="里程（公里）" prop="km">
          <el-input
            v-model="dailyTrainStationForm.km"
