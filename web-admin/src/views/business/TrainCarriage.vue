...
const handleDelete = async (ids) => {
  try {
    const res = await deleteTrainCarriage(ids)
    if (res.code === 200) {
      ElMessage.success('删除成功')
      // 如果当前页只剩要被删除的条数且不是第一页，删除后自动跳转到上一页
      if (trainCarriageList.value.length === ids.length && pagination.page > 1) {
        pagination.page--
      }
      await getTrainCarriageList()
    } else {
...
const getTrainCarriageList = async () => {
  ...
  try {
    ...
    ElMessage.error(error.response?.data?.msg || '获取火车车厢列表失败')
  } finally {
